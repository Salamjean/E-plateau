...
use App\Models\LivraisonExtrait;
...
class LivraisonExtraitController extends Controller
{
    ...
    public function attribuerDemande(Request $request)
    {
        ...
        if ($demande->livraison_id == null) {
            return back()->with('error', 'Ce colis n\'est pas dans notre service.');
        }
        ...
        // Si le colis n'est pas encore attribué mais répond aux conditions
        return back()->with('error', 'Ce colis est disponible pour enregistrement.');
    }
    ...
    public function demandesAttribuees()
    {
        ...
        foreach ($modeles as $modele) {
            $classeModele = "App\\Models\\$modele";
            $demandesModele = $classeModele::where('livraison_id', $poste->id)
    ...
    {
        $types = [
            'NaissanceSimple' => 'NaissanceSimple',
            'NaissanceCertificat' => 'NaissanceCertificat',
            'DecesSimple' => 'DecesSimple',
            'DecesCertificat' => 'DecesCertificat',
            'Mariage' => 'Mariage'
        ];
