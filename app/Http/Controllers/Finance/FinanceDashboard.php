...
class FinanceDashboard extends Controller
{
    public function dashboard(){
        ...
        $today = Carbon::today();
        $decesAujourdhui = DecesCertificat::where('commune', $admin->communeM)
            ->whereDate('created_at', $today)
            ->count();
        $decesdejaAujourdhui = DecesSimple::where('commune', $admin->communeM)
            ->whereDate('created_at', $today)
            ->count();
        $mariageAujourdhui = Mariage::where('commune', $admin->communeM)
            ->whereDate('created_at', $today)
            ->count();
        $naissanceAujourdhui = NaissanceCertificat::where('commune', $admin->communeM)
            ->whereDate('created_at', $today)
            ->count();
        $naissanceDAujourdhui = NaissanceSimple::where('commune', $admin->communeM)
            ->whereDate('created_at', $today)
            ->count();
        $totalAujourdhui = $decesAujourdhui + $decesdejaAujourdhui + $naissanceAujourdhui + $naissanceDAujourdhui + $mariageAujourdhui;

        // Statistiques de la semaine en cours
        $weekStart = Carbon::now()->startOfWeek();
        $weekEnd = Carbon::now()->endOfWeek();
        
        $decesSemaine = DecesCertificat::where('commune', $admin->communeM)
            ->whereBetween('created_at', [$weekStart, $weekEnd])
            ->count();
        $decesdejaSemaine = DecesSimple::where('commune', $admin->communeM)
            ->whereBetween('created_at', [$weekStart, $weekEnd])
            ->count();
        $mariageSemaine = Mariage::where('commune', $admin->communeM)
            ->whereBetween('created_at', [$weekStart, $weekEnd])
            ->count();
        $naissanceSemaine = NaissanceCertificat::where('commune', $admin->communeM)
            ->whereBetween('created_at', [$weekStart, $weekEnd])
            ->count();
        $naissanceDSemaine = NaissanceSimple::where('commune', $admin->communeM)
            ->whereBetween('created_at', [$weekStart, $weekEnd])
            ->count();
        $totalSemaine = $decesSemaine + $decesdejaSemaine + $naissanceSemaine + $naissanceDSemaine + $mariageSemaine;
        ...
        return view('finance.dashboard',
            compact(
                ...
                'totalAujourdhui', 'totalSemaine','timbresAujourdhui', 'montantAujourdhui',
