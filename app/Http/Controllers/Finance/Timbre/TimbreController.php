...
{
   // Dans votre contrôleur TimbreController
   ...
   public function create()
   {
      ...
      return view('finance.timbre.create', compact(
         'solde_timbres', 
         'dernieres_ventes',
         'ventes_aujourdhui',
         'ventes_semaine',
  ...
  public function history(Request $request)
   {
      ...
      $stats = [
         'today' => abs(Timbre::where('finance_id', $financeId)
                     ->where('nombre_timbre', '<', 0)
                     ->whereDate('created_at', today())
                     ->sum('nombre_timbre')),
         
         'this_week' => abs(Timbre::where('finance_id', $financeId)
                     ->where('nombre_timbre', '<', 0)
                     ->whereBetween('created_at', [now()->startOfWeek(), now()->endOfWeek()])
                     ->sum('nombre_timbre')),
         
         'this_month' => abs(Timbre::where('finance_id', $financeId)
                     ->where('nombre_timbre', '<', 0)
                     ->whereBetween('created_at', [now()->startOfMonth(), now()->endOfMonth()])
                     ->sum('nombre_timbre'))
      ];
      
      return view('finance.timbre.history', compact('ventes', 'total_ventes', 'stats'));
   }
}
