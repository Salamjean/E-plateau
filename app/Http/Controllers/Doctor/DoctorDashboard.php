    ...
    public function dashboard(Request $request)
        {
            try {
                ...
                $declarationsRecents = DeclarationNaissance::where('NomEnf', $communeAdmin)
                ...
                $decesRecents = DeclarationDeces::where('nomHop', $communeAdmin)
                    ->where('doctor_id', $sousAdminId) // Filtrer par ID de sous-administrateur
                    ->whereDate('created_at', now()->format('Y-m-d')) // Filtrer par date
                    ->orderBy('created_at', 'desc')
                    ->take(3)
                ...
                $naisshopData = DeclarationNaissance::select(DB::raw("DATE_FORMAT(created_at, '%Y-%m') as month"), DB::raw('count(*) as count'))
                ...
                $deceshopData = DeclarationDeces::select(DB::raw("DATE_FORMAT(created_at, '%Y-%m') as month"), DB::raw('count(*) as count'))
