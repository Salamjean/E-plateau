...
class DecesSimpleConroller extends Controller
{
    ...
    public function store(Request $request, InfobipService $infobipService)
    {
        $request->validate([
            ...
            'pActe.required' => 'Cet document de décès est obligatoire.',
        ...
        $reference = 'ADJ' . str_pad(DecesSimple::getNextId(), 4, '0', STR_PAD_LEFT) . $communeInitiale . $anneeCourante; // ADJ pour Acte de Decès Déjà Jugé


        // Enregistrement de l'objet Decesdeja
        $decesdeja = new DecesSimple();
...
Vous pouvez suivre l'état de votre demande en cliquant sur ce lien : https://plateau-apps.com/home/search";
    ...
    public function delete(DecesSimple $decesdeja)
    {
        try {
            $decesdeja->delete();
